...
INSERT INTO _schema_log(version_number, update_by,update_at,description) 
  VALUES ('01.01.10.0031', 'tecs-vnm_test',now(),'first version');

alter table vnic add column ass_ip character varying( 64 );
alter table vnic add column ass_mask character varying( 64 );
alter table vnic add column ass_gateway character varying( 64 );
...
CREATE OR REPLACE FUNCTION pf_net_check_add_vsi(in_vm_id numeric(19,0),in_nic_index integer,
                           ...
                           in_ass_gateway character varying,                           
...
BEGIN
  ...
  result.ret := pf_net_check_add_vsi(in_vm_id,in_nic_index, in_is_sriov, in_is_loop, in_logicnetwork_uuid,
                      in_ass_ip, in_ass_mask, in_ass_gateway,
                      in_port_type,
                      in_vsi_id_value,in_vsi_id_format,in_vm_pg_uuid,in_mac,in_is_has_ip,in_ip,
                      in_is_has_vswitch,in_vswitch_uuid,in_is_has_sriovvf,in_sriov_vf_port_uuid,
  ...
  BEGIN 
      ...
      -- INSERT VNIC 
     INSERT INTO vnic(id, vm_id, nic_index, is_sriov ,is_loop, logic_network_uuid, ass_ip, ass_mask, ass_gateway, port_type, 
             vsi_profile_id )
      VALUES(vnicid,in_vm_id,in_nic_index,in_is_sriov,in_is_loop,in_logicnetwork_uuid, in_ass_ip, in_ass_mask, in_ass_gateway, in_port_type, 
             result.id);
