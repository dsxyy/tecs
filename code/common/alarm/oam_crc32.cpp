...
void InitCRC32Table(void)
{
    ...
    WORD32 dwPolynomial = 0x04c11db7;
    WORD32 i = 0, j = 0;
    
    /* 256 values representing ASCII character codes. */
    for (i = 0; i <= 0xFF; i++) 
    { 
        g_adwOamCRC32Table[i]=Reflect(i, 8) << 24; 
        for (j = 0; j < 8; j++) 
        {    
            g_adwOamCRC32Table[i] = (g_adwOamCRC32Table[i] << 1) 
                ^ (g_adwOamCRC32Table[i] & (1 << 31) ? dwPolynomial : 0);
        } 
        g_adwOamCRC32Table[i] = Reflect(g_adwOamCRC32Table[i], 32); 
    } 
}

WORD32 Reflect(WORD32 dwRef, BYTE ucCount)
{
...
WORD32 Oam_CountCRC32(BYTE *pucData , WORD32 dwLen, WORD32 dwPreCRC)
{
    ...
    while(dwLen--)
    {
        dwCRC = (dwCRC >> 8) ^ g_adwOamCRC32Table[(dwCRC & 0xFF) ^ *pucData++];
    }
