...
int ataPrintSmartSelfTestlog(struct ata_smart_selftestlog *data,int allentries){
  ...
  for (i=20;i>=0;i--){    
    ...
    {
      const char *msgtest = NULL;
      const char *msgstat = NULL;
      char percent[64],firstlba[64];
      int errorfound=0;
...
int ataPrintMain (int fd){
  ...
  // Enable/Disable SMART commands
    ...
    //##############################################################  
    // add by kangzhenhua 2011.03.21 for 如果磁盘支持smart却没有开启，则开启
    //##############################################################
    #ifndef SMART_CTL_TOOL
  ...
  g_tSmartInfo->sdwSelfTestTime = (int)timewait;
  #endif
      } else {
	t+=timewait*60;
  ...
  #ifndef SMART_CTL_TOOL
  //printf("complete after %d seconds \n", (int)timewait*60);
  g_tSmartInfo->sdwSelfTestTime = (int)timewait*60;
  #endif
      }
