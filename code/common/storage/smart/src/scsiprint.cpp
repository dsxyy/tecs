...
extern T_DmPdSmartInfo *g_tSmartInfo;
...
static int scsiGetSmartData(int device, int attribs)
{
    ...
    if (scsiCheckIE(device, gSmartLPage, gTempLPage, &asc, &ascq,
        ...
        PRINT_OFF(con);
        //##############################################################
        //  close by kangzhenhua 2011.03.21 for 查一批SMART的时候，从此处返回
        //  稍后需要研究为啥 ???????????????????????? 
        //#############################################################
        return -1;
    }
    ...
    if (cp) {
        ...
        //##############################################################
        //       add by kangzhenhua 2011.02.23 for SMART状态是否OK  
        //#############################################################
        #ifndef SMART_CTL_TOOL
        g_tSmartInfo->ucSmartStatus = PD_BROKEN;
        #endif
        PRINT_OFF(con);
    } else if (gIecMPage)
     {
        ...
        //#############################################################
        #ifndef SMART_CTL_TOOL
        g_tSmartInfo->ucSmartStatus = PD_GOOD;
        #endif
     }
...
// TapeAlerts fails
static const char *severities = "CWI";
...
static int scsiGetDriveInfo(int device, UINT8 * peripheral_type, int all)
{
    ...
    int peri_dt = 0;
    int returnval=0;
    int SmartEnable = 0;      //add by kangzhenhua 2011.03.21
    ...
    //##############################################################  
    //        closed by kangzhenhua 2011.03.21 for 流程走下去，获取是否支持SMART
    //##############################################################
    //if (! all)
    //    return 0;
    ...
    if (iec_err) {
        if (!is_tape) {
            ...
            //##############################################################  
            //        add by kangzhenhua 2011.02.23 for 是否支持SMART
            //##############################################################
            #ifndef SMART_CTL_TOOL
            g_tSmartInfo->ucIsSupportSmart = PD_NOT_SUPPORT_SMART;
            #endif
            if (con->reportscsiioctl > 0)
    ...
    if (!is_tape)
    {
        SmartEnable = scsi_IsExceptionControlEnabled(&iec);
        //##############################################################  
        //        changed by kangzhenhua 2011.03.21 for 获取磁盘是否支持SMART、SMART是否开启
        //##############################################################
        pout("Device supports SMART and is %s\n",
             (SmartEnable) ? "Enabled" : "Disabled");
        //##############################################################  
        //        add by kangzhenhua 2011.02.23 for 是否支持SMART
        //##############################################################
        #ifndef SMART_CTL_TOOL
        g_tSmartInfo->ucIsSupportSmart = PD_SUPPORT_SMART;
        //##############################################################  
        //       add by kangzhenhua 2011.02.23 for SMART功能是否开启
        //##############################################################
        if(SmartEnable)
        {
            g_tSmartInfo->ucIsSmartEnable = PD_SMART_ON;
        }
        else
        {
            g_tSmartInfo->ucIsSmartEnable = PD_SMART_OFF;
        }
        #endif
    }
...
static void scsi_init_global_var(void)
{
    gSmartLPage = 0;     /* Informational Exceptions log page */
    gTempLPage = 0;
    gSelfTestLPage = 0;
    gStartStopLPage = 0;
    gReadECounterLPage = 0;
    gWriteECounterLPage = 0;
    gVerifyECounterLPage = 0;
    gNonMediumELPage = 0;
    gLastNErrorLPage = 0;
    gBackgroundResultsLPage = 0;
    gTapeAlertsLPage = 0;
    gSeagateCacheLPage = 0;
    gSeagateFactoryLPage = 0;
    
    /* Mode pages supported */
    gIecMPage = 1;     /* N.B. assume it until we know otherwise */
...
int scsiPrintMain(int fd)
{
    ...
    //##############################################################  
    //  changed by kangzhenhua 2011.03.21 for 如果SMART未开启，开启它
    //##############################################################
    #ifndef SMART_CTL_TOOL
        if((!g_tSmartInfo->ucIsSmartEnable) && (g_tSmartInfo->ucIsSupportSmart))
    ...
    if (con->smartextendselftest) {
        ...
        if ((0 == scsiFetchExtendedSelfTestTime(fd, &durationSec, 
            ...
            pout("Please wait %d minutes for test to complete.\n", 
                 durationSec / 60);
            //##############################################################
            //       add by kangzhenhua 2011.03.04 for 打印执行时间  
            //#############################################################
            #ifndef SMART_CTL_TOOL
            //printf("complete after %d seconds \n", durationSec);
            g_tSmartInfo->sdwSelfTestTime = durationSec;
            #endif
