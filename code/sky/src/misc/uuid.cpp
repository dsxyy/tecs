...
#if 0 /* gongxf del for warning not used*/   
...
void uuid_create_sha1_from_name(uuid_t *uuid, uuid_t nsid, void *name,
                                int namelen)
{
#if 0  /*gongxf del depend sha_ctx */
    SHA_CTX c;
...
/* format_uuid_v3or5 -- make a UUID from a (pseudo)random 128-bit
   number */
#if 0   /*gongxf del for compiler warning not used*/
void format_uuid_v3or5(uuid_t *uuid, unsigned char hash[16], int v)
{
    /* convert UUID to local byte order */
    memcpy(uuid, hash, sizeof *uuid);
    uuid->time_low = vnet_ntohl(uuid->time_low);
    uuid->time_mid = vnet_ntohs(uuid->time_mid);
    uuid->time_hi_and_version = vnet_ntohs(uuid->time_hi_and_version);
