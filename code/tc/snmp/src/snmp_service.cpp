...
netsnmp_register_default_domain(const char* application, const char* domain)
{
	...
	run = (struct netsnmp_lookup_domain *)malloc(sizeof(struct netsnmp_lookup_domain));
    if (run == NULL)
        return 1;
				...
				const char* target)
{
	...
	run = (struct netsnmp_lookup_target *)malloc(sizeof(struct netsnmp_lookup_target));
    if (run == NULL)
        return -1;
	run->application = strdup(application);
	run->domain = strdup(domain);
