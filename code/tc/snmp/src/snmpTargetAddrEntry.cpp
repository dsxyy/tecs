...
struct targetAddrTable_struct *
get_addrForAddress(int ipaddress,int port)
{
    int dwTrapPort = 0;
    struct targetAddrTable_struct *ptr;
    for (ptr = aAddrTable; ptr != NULL; ptr = ptr->next) 
    {
        dwTrapPort = *(ptr->tAddress+4)*256+*(ptr->tAddress+5);
        if ((ptr->tAddress[0] == ((ipaddress & 0xff000000) >> 24)) &&
            (ptr->tAddress[1] == ((ipaddress & 0x00ff0000) >> 16)) &&
            (ptr->tAddress[2] == ((ipaddress & 0x0000ff00) >> 8)) &&
            (ptr->tAddress[3] == (ipaddress & 0x000000ff)) &&
            (port == dwTrapPort))
        {
            return ptr;
        }
...
void snmp_show_trapaddr()
{
    unsigned int dwTrapPort = 0;
    struct targetAddrTable_struct *ptr;
    printf("name          port    address\n");
    for (ptr = aAddrTable; ptr != NULL; ptr = ptr->next) 
    {
        dwTrapPort = *(ptr->tAddress+4)*256+*(ptr->tAddress+5);
        printf("%4s    %4u     %d.%d.%d.%d\n", ptr->name, dwTrapPort, ptr->tAddress[0], ptr->tAddress[1], ptr->tAddress[2], ptr->tAddress[3]);
    }
}
...
void
snmpTargetAddrTable_remove(struct targetAddrTable_struct *oldEntry)
{
    snmpTargetAddrTable_remFromList(oldEntry,&aAddrTable);
}
