...
int snmpNotifyVersion = SNMP_VERSION_2c;
/*
...
send_notifications(int major, int minor, void *serverarg, void *clientarg)
{
    ...
    for (hptr = snmpNotifyTableStorage; hptr; hptr = hptr->next) {
        ...
        for (sptr = sess; sptr; sptr = sptr->next) {
            ...
            if (snmpNotifyVersion == SNMP_VERSION_1 &&
                minor == SNMPD_CALLBACK_SEND_TRAP1) {
                send = 1;
            } else
#endif
            if ((snmpNotifyVersion == SNMP_VERSION_3
...
int GetTrapVersion()
{
	return snmpNotifyVersion;
}

int SetTrapVersion(int TrapVersion)
{
    struct targetParamTable_struct *ptr;
    struct targetAddrTable_struct *ptrAddr;
	
    snmpNotifyVersion = TrapVersion;

    for (ptrAddr = get_addrTable();ptrAddr != 0; ptrAddr = ptrAddr->next)
    {
        ptrAddr->sess->version = snmpNotifyVersion;
    }

    for (ptr = get_aPTable();ptr != 0; ptr =ptr->next)
    {
        ptr->mpModel = snmpNotifyVersion;
        ptr->secModel = snmpNotifyVersion + 1;
    }
