...
_init_snmp(void)
{
    ...
    tmpReqid = (long)random();
    tmpMsgid = (long)random();
#endif
...
snmp_create_sess_pdu(netsnmp_transport *transport, void *opaque,
                     size_t olength)
{
    netsnmp_pdu *pdu = (netsnmp_pdu *)malloc(1*sizeof(netsnmp_pdu));
    if (pdu == NULL) {
...
_sess_read(void *sessp, fd_set * fdset)
{
    ...
    size_t          pdulen = 0, rxbuf_len = 8096;
    u_char         *rxbuf = NULL;
    ...
    if (transport->flags & NETSNMP_TRANSPORT_FLAG_STREAM) 
    {
        ...
        else
        {
            ...
            {
                newbuf =
                    (u_char *) malloc(isp->packet_len + rxbuf_len);
                if (newbuf == NULL) 
                {
                    DEBUGMSGTL(("sess_read",
                ...
                {
                    memset(newbuf,0,isp->packet_len + rxbuf_len);
                    memcpy(newbuf,isp->packet,strlen((char *)isp->packet));
                    isp->packet = newbuf;
    ...
    else 
    {
        ...
        memset(rxbuf,0,sizeof(rxbuf_len));
    }
	...
	/*  If we had more than one packet, then we were working with copies
        ...
        if ((rxbuf = (u_char *)malloc(isp->packet_len)) == NULL) {
            /*
        ...
        else 
        {
            DEBUGMSGTL(("sess_read", "realloc() okay, old buffer %p, new %p\n",
                        isp->packet, rxbuf));
	    memset(rxbuf,0,sizeof(isp->packet_len));
            memcpy(rxbuf,isp->packet,strlen((char *)isp->packet));
            isp->packet = rxbuf;
...
int _check_range(struct tree *tp, long ltmp, int *resptr,
	             const char *errmsg)
{
...
/**********************************************************************
* snmp_rebuild_Sessions
* 功能描述：在Sessions列表中查询本次配置修改的session，重新创建对应transpsort
* 访问的表：无
* 修改的表：无
* 输入参数：
      sess - session；t-新创建的transport
* 输出参数：无
* 返 回 值：
          无
* 其它说明：无
* 修改日期      版本号  修改人      修改内容
* ---------------------------------------------------------------------
* 2008/01/04    V1.0    安志奇      创建
************************************************************************/
void
snmp_rebuild_Sessions(netsnmp_session *sess,netsnmp_transport *t)
{
    struct session_list *slp;
    void           *sessp;

    sessp = snmp_sess_pointer(sess);
    slp = (struct session_list *) sessp;
    free(slp->transport);
