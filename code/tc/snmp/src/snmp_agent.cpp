...
#include "snmpd.h"
#include "struct.h"
#include "mibgroup/utilities/util_funcs.h"
...
/*  add this method to update table data before handle_var_request
     add by anzhq 2008.01.31 */
void 
update_tablevar_data(netsnmp_agent_session *asp)
{
    int             i;
    ...
    for (i = 0; i <= asp->treecache_num; i++) {
        ...
        {
            handler = asp->treecache[i].subtree->reginfo->handler;
            for (;handler;handler = handler->next)
            {
                if (!strcmp(handler->handler_name,TABLE_ITERATOR_NAME))
                {
                    iinfo = (netsnmp_iterator_info *) handler->myvoid;
#if 0
                    if (check_tablevar_data(asp->pdu->variables) == 1)
                    {
                        if(NULL != iinfo->get_all_data_point)
                            iinfo->get_all_data_point(iinfo->handler_name);
                    }
#endif
                }
            }
...
/**********************************************************************
* check_tablevar_data
* 功能描述：检测是否getbulk第一包数据
* 访问的表：无
* 修改的表：无
* 输入参数：netsnmp_variable_list
* 输出参数：
* 返 回 值：0 -整表访问，调用函数访问业务数据；
                             1 -非整表或错误，不访问数据
* 其它说明：CRDCR00297765
           CRDCR00313917  
* 修改日期      版本号  修改人      修改内容
* ---------------------------------------------------------------------
* 2008/09/01    V1.0    anzhq     创建
************************************************************************/
int check_tablevar_data(netsnmp_variable_list *vars)
{
    oid  cpuORmem_oid[] = { 1, 3, 6, 1, 4, 1, 3902, 150, 11, 5, 10, 12, 1 };
    oid  ub_oid[] =       { 1, 3, 6, 1, 4, 1, 3902, 150, 11, 5, 10, 12, 2 };
    oid  ethernet_oid[] = { 1, 3, 6, 1, 4, 1, 3902, 150, 11, 5, 10, 12, 3 };
    oid  ipmb_oid[] =     { 1, 3, 6, 1, 4, 1, 3902, 150, 11, 5, 10, 12, 4 };
    oid  alarminfo_oid[] = { 1, 3, 6, 1, 4, 1, 3902, 150, 11, 5, 10, 11, 3 }; 

    size_t  oidLen = OID_LENGTH(cpuORmem_oid);
    ...
    if (0 == snmp_oid_compare(vars->name_loc,oidLen,cpuORmem_oid,oidLen) ||
        0 == snmp_oid_compare(vars->name_loc,oidLen,ub_oid,oidLen) ||
        0 == snmp_oid_compare(vars->name_loc,oidLen,ethernet_oid,oidLen) ||
        0 == snmp_oid_compare(vars->name_loc,oidLen,ipmb_oid,oidLen) ||
        0 == snmp_oid_compare(vars->name_loc,oidLen,alarminfo_oid,oidLen) )
    {
        if(vars->name_length > oidLen)
            return 0;
    } 
...
handle_pdu(netsnmp_agent_session *asp)
{
    ...
    case SNMP_MSG_GET:
		...
		/*
         * 如果是查询某个表数据，则先更新表记录
         * edit by caoliang in Feb 20th
         */
        update_tablevar_data(asp);
...
_request_set_error(netsnmp_request_info *request, int mode, int error_value)
{
    ...
    case SNMP_ENDOFMIBVIEW:
        ...
        default:
            ...
            request->requestvb->type = error_value; /* by liuhzh */
            return SNMPERR_SUCCESS;
        }
