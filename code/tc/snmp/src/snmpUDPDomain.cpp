...
char g_community[VACMSTRINGLEN] = {0};
			...
			char *data, int len)
{
    ...
    if(srcip)
        memcpy((void *)&cmsg.ipi.ipi_spec_dst,(void *)&srcip->s_addr,sizeof(srcip->s_addr));
    else
        memset((void *)&cmsg.ipi.ipi_spec_dst,0,sizeof(cmsg.ipi.ipi_spec_dst));
		 ...
		 void **opaque, int *olength)
{
	...
	while (rc < 0) {
#if defined(linux) && defined(IP_PKTINFO)
            rc = netsnmp_udp_recvfrom(t->sock, (char *)buf, size, from, &fromlen, &(addr_pair->local_addr));
#else
		 ...
		 void **opaque, int *olength)
{
	...
	while (rc < 0) {
#if defined(linux) && defined(IP_PKTINFO)
            rc = netsnmp_udp_sendto(t->sock, addr_pair ? &(addr_pair->local_addr) : NULL, to, (char *)buf, size);
#else
...
void netsnmp_udp_show_security()
{
    ...
    com2SecEntry   *node = NULL;
    printf("index,    community,        network,            secName\n");
    printf("=======================================================\n"); 
    node = com2SecList;
    while(node != NULL){
        ip_addr.s_addr = node->network;
        printf("%5d,    ", ++pos);
        printf("%9s,    ", node->community);
...
char *
snmp_udp_get_community(in_addr_t network)
{    
    com2SecEntry   *curr = com2SecList;
    while(curr != NULL){
        if (network == curr->network){
            return curr->community;
        }
        else
        {
            curr = curr->next;
...
void            
netsnmp_udp_del_security(in_addr_t network, char *strCommunity)
{  
    /* delete from list */
    if (com2SecList == NULL){ /* 链表空 */
        com2SecListLast = NULL;
        return;
    }
    else{
        com2SecEntry   *prior = NULL;
        com2SecEntry   *curr = com2SecList;
        while(curr != NULL){
            if (network == curr->network){
                strcpy(strCommunity, curr->community);
                if (curr == com2SecList){ /* 删除的是第一个*/
                    if (curr == com2SecListLast){ /* 同时也是最后一个*/
                        com2SecList = NULL;
                        com2SecListLast = NULL;
                    }
                    else
                    {
                        com2SecList = curr->next;
                    }
                }
                else if (curr == com2SecListLast){ /* 删除的是最后一个*/
                    com2SecListLast = prior;
                    prior->next = NULL;
...
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
static char *g_szDefSecName = (char *)"mynetwork";
static char *g_szDefContextName = (char *)"";
int
                       ...
                       char **contextName)
{
    ...
    if (secName != NULL) {
        *secName = g_szDefSecName;
        *contextName = g_szDefContextName;    
    }
    /*
        1、若没有配置数据，community按照platform默认值进行限制;
    */
    if (com2SecList == NULL){
        if (memcmp(community, "platform", community_len) == 0){
    ...
    /* 判断community*/
    if (memcmp(community, g_community, community_len) == 0) 
    {
        DEBUGMSGTL(("netsnmp_udp_getSecName", "no com2sec entries\n"));
        return 1;
    }
