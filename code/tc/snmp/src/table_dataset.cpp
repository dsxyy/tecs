    ...
    (netsnmp_table_data_set_storage *defrow)
{
    ...
    for (; defrow; defrow = defrow->next) {
        netsnmp_set_row_column(row, defrow->column, defrow->type,
                               (char *)defrow->data.voidp, defrow->data_len);
        if (defrow->writable)
                                      ...
                                      netsnmp_request_info *requests)
{
    ...
    for (request = requests; request; request = request->next) {
        ...
        case MODE_SET_RESERVE2:
            ...
            data->data.string =
                (u_char *)netsnmp_strdup_and_null(request->requestvb->val.string,
                                 ...
                                 int writable)
{
    ...
    if (!data) {
        ...
        data->next = (netsnmp_table_data_set_storage *)row->data;
        row->data = data;
    } else {
        data->writable = writable;
    }
                       ...
                       int type, const char *value, size_t value_len)
{
    ...
    if (value) {
        ...
        if (value_len) {
            ...
            data->data.string = (u_char *)malloc(1);
        }
