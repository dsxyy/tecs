...
netsnmp_cache_create(int timeout, NetsnmpCacheLoad * load_hook,
                     NetsnmpCacheFree * free_hook, NetsnmpCacheCommit *commit_hook,
                     oid * rootoid, int rootoid_len)
{
    ...
    cache->commit_cache = commit_hook;
    cache->enabled = 1;
...
void netsnmp_cache_free_myvoid(void **myvoid)
{
    netsnmp_cache  *cache = (netsnmp_cache  *)*myvoid;
                          ...
                          oid * rootoid, int rootoid_len)
{
    ...
    if (ret) {
        cache = netsnmp_cache_create(timeout, load_hook, free_hook, commit_hook,
                                     rootoid, rootoid_len);
        ret->myvoid = (void *) cache;
        ret->free_myvoid_method = netsnmp_cache_free_myvoid;
    }
    return ret;
}
...
netsnmp_register_cache_handler(netsnmp_handler_registration * reginfo,
                               ...
                               NetsnmpCacheCommit * commit_hook)
{
    netsnmp_mib_handler *handler = NULL;
    handler = netsnmp_get_cache_handler(timeout, load_hook, free_hook, commit_hook,
                                        reginfo->rootoid,
                                        reginfo->rootoid_len);
                             ...
                             netsnmp_request_info * requests)
{
    ...
    case MODE_SET_RESERVE1: {
             ...
             */
            if (netsnmp_cache_check_and_reload(cache) < 0)
            {
                break;
            }
    ...
    switch (reqinfo->mode) {
        ...
        case MODE_SET_COMMIT:
            if (cache->valid) {
                if (cache->commit_cache != NULL){
                    if (cache->commit_cache(cache, cache->magic) < 0){
