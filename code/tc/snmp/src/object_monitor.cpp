                         ...
                         netsnmp_object_monitor_callback * cb)
{
    ...
    mi = (monitor_info   *)calloc(1, sizeof(monitor_info));
    if (NULL == mi)
...
netsnmp_notify_monitor(netsnmp_monitor_callback_header * cbh)
{
     ...
     */
    cbh->priv = callback_pending_list;
                           ...
                           void *object_info)
{
    ...
    if (o_steal) {
        ...
        if (cbh->hdr.monitored_object.oids == NULL)
            return;
    }
...
insert_watcher(oid * object, size_t oid_len, monitor_info * mi)
{
    ...
    if (NULL != wl) {
        ...
        wl->monitored_object.oids = (oid *)malloc(sizeof(oid) * oid_len);
...
move_pending_to_ready(void)
{
    ...
    while (callback_pending_list) {
        ...
        callback_pending_list = cbp->priv; /** next */
