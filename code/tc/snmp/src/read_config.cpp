...
#if 0
static int writeconfileflag = 0;
char snmpconfText[MAX_SNMPCONFTEXT_LEN] = {0};

/*-- 配置文件中TRAP相关参数默认值  --*/
 char SNMPTrapCommunity[STRING_OID_MAXLEN] = "public";
 char SNMPTrapAddress1[STRING_OID_MAXLEN] = "10.44.41.19";
 char SNMPTrapAddress2[STRING_OID_MAXLEN] = "10.44.36.231";
 char SNMPTrapAddress3[STRING_OID_MAXLEN] = "10.0.0.0";
 char SNMPTrapAddress4[STRING_OID_MAXLEN] = "10.0.0.0";
 char SNMPTrapAddress5[STRING_OID_MAXLEN] = "10.0.0.0";
 char SNMPTrapPort1[20] = "162";
 char SNMPTrapPort2[20] = "162";
 char SNMPTrapPort3[20] = "162";
 char SNMPTrapPort4[20] = "162";
 char SNMPTrapPort5[20] = "162";

 /*TRAP配置相关节点*/
 char sysOperationName[STRING_OID_MAXLEN] = "Linux";
 char sysNetElementType[STRING_OID_MAXLEN] = "CMM";
 char sysNetElementName[STRING_OID_MAXLEN] = "CMM";

void setTrapCommunity(char * TrapCommunity,size_t Community_len)
{
    if(NULL == TrapCommunity)
	return;
	
    if(Community_len < STRING_OID_MAXLEN)
        sprintf(SNMPTrapCommunity, "%s",TrapCommunity);
}

void setTrapPort(int index,unsigned int Portvalue)
{
    if (index == 0)
    {
        sprintf(SNMPTrapPort1, "%d",Portvalue);
    }
    else if (index == 1)
    {
        sprintf(SNMPTrapPort2, "%d",Portvalue);
    }
    else if (index == 2)
    {
        sprintf(SNMPTrapPort3, "%d",Portvalue);
    }
    else if (index == 3)
    {
        sprintf(SNMPTrapPort4, "%d",Portvalue);
    }
    else if (index == 4)
    {
        sprintf(SNMPTrapPort5, "%d",Portvalue);
    }
}

void setTrapAddress(int index,unsigned char * TrapAddr,size_t Addr_len)
{
    if(NULL == TrapAddr)
	return;
    ...
    if (index == 0)
    {
        sprintf(SNMPTrapAddress1, "%d.%d.%d.%d",(TrapAddr[0]), (TrapAddr[1]),(TrapAddr[2]),(TrapAddr[3]));
    }
    else if (index == 1)
    {
        sprintf(SNMPTrapAddress2, "%d.%d.%d.%d",(TrapAddr[0]), (TrapAddr[1]),(TrapAddr[2]),(TrapAddr[3]));
    }
    else if (index == 2)
    {
        sprintf(SNMPTrapAddress3, "%d.%d.%d.%d",(TrapAddr[0]), (TrapAddr[1]),(TrapAddr[2]),(TrapAddr[3]));
    }
    else if (index == 3)
    {
        sprintf(SNMPTrapAddress4, "%d.%d.%d.%d",(TrapAddr[0]), (TrapAddr[1]),(TrapAddr[2]),(TrapAddr[3]));
    }
    else if (index == 4)
    {
        sprintf(SNMPTrapAddress5, "%d.%d.%d.%d",(TrapAddr[0]), (TrapAddr[1]),(TrapAddr[2]),(TrapAddr[3]));
    }
    else
        return;
}
...
get_persistent_directory()
{
    if (NULL == netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID, 
        ...
        if (NULL == persdir)
            persdir = (char *)NETSNMP_PERSISTENT_DIRECTORY;
...
WriteConfigFile(const char *filename)
{
    ...
    char TrapCommunity[STRING_OID_MAXLEN*2] = {0};
    char TrapAddress1[STRING_OID_MAXLEN*2] = {0};
    char TrapAddress2[STRING_OID_MAXLEN*2] = {0};
    char TrapAddress3[STRING_OID_MAXLEN*2] = {0};
    char TrapAddress4[STRING_OID_MAXLEN*2] = {0};
    char TrapAddress5[STRING_OID_MAXLEN*2] = {0};
    char OperationName[STRING_OID_MAXLEN*2] = {0};
    char NetElementName[STRING_OID_MAXLEN*2] = {0};
    char NetElementType[STRING_OID_MAXLEN*2] = {0};
    int TrapVersion = 0;
    T_Snmp_ConfData  tsnmpSyndata = {0};
	...
	return;
    /* 创建或更新文件 */
    if (writeconfileflag == 0)
    {
        if((fp1 = fopen(filename,"w")) == NULL)
        {   
            OAM_Printf(PRINT_OAM_SNMP,PRN_NORMAL,PRN_LEVEL_HIGHEST,
				"SNMPAgt():create file fail!\n");
            return -1;
        }
    } 
    else
    ...
    /* 构造要写入的内容 */
    /* 默认部分，这部分以后也要动态生成，如果用户在外面打开文件修改了 */
    confText =    "#       sec.name  source          community\n"
                  "com2sec local 127.0.0.1 public\n"
                  "com2sec mynetwork 128.254.31.0/24 public\n"
                  "com2sec mynetwork 10.44.124.0/24  public\n"
                  "com2sec mynetwork 10.44.41.0/24   public\n"
                  "com2sec mynetwork 10.44.40.0/24   public\n"
                  "com2sec mynetwork 10.40.70.0/24   public\n"
                  "com2sec mynetwork 10.44.37.0/24   public\n"
                  "com2sec mynetwork 10.44.126.0/24  public\n"
                  "com2sec mynetwork 10.44.115.0/24  public\n"
                  "\n"
    ...
    warnText = "############################################################################\n"
               "# STOP STOP STOP STOP STOP STOP STOP STOP STOP \n"
               "#\n"
               "#          **** DO NOT EDIT THE LATTER TOKENS ****\n"
               "#\n"
               "# STOP STOP STOP STOP STOP STOP STOP STOP STOP \n"
               "############################################################################\n";

    if (writeconfileflag != 0)  /* 不是创建 */
    {
        snmp_getSynData(&tsnmpSyndata);

        setTrapCommunity(tsnmpSyndata.trapCommunity,strlen(tsnmpSyndata.trapCommunity));
        TrapVersion = tsnmpSyndata.trapVersion;
        setTrapAddress(0,tsnmpSyndata.trapAddr1,4);
        setTrapAddress(1,tsnmpSyndata.trapAddr2,4);
        setTrapAddress(2,tsnmpSyndata.trapAddr3,4);
        setTrapAddress(3,tsnmpSyndata.trapAddr4,4);
        setTrapAddress(4,tsnmpSyndata.trapAddr5,4);
        setTrapPort(0,tsnmpSyndata.trapPort[0]);
        setTrapPort(1,tsnmpSyndata.trapPort[1]);
        setTrapPort(2,tsnmpSyndata.trapPort[2]);
        setTrapPort(3,tsnmpSyndata.trapPort[3]);
        setTrapPort(4,tsnmpSyndata.trapPort[4]);  
    }

    sprintf(TrapCommunity, "trapcommunity %s\n", SNMPTrapCommunity);
    if(TrapVersion == SNMP_VERSION_2c)
    {
        sprintf(TrapAddress1, "trap2sink %s:%-15s\n", SNMPTrapAddress1,SNMPTrapPort1);
        sprintf(TrapAddress2, "trap2sink %s:%-15s\n", SNMPTrapAddress2,SNMPTrapPort2);
        sprintf(TrapAddress3, "trap2sink %s:%-15s\n", SNMPTrapAddress3,SNMPTrapPort3);
        sprintf(TrapAddress4, "trap2sink %s:%-15s\n", SNMPTrapAddress4,SNMPTrapPort4);
        sprintf(TrapAddress5, "trap2sink %s:%-15s\n\n", SNMPTrapAddress5,SNMPTrapPort5);
    }
    else
    {
        sprintf(TrapAddress1, "trapsink %s:%-16s\n", SNMPTrapAddress1,SNMPTrapPort1);
        sprintf(TrapAddress2, "trapsink %s:%-16s\n", SNMPTrapAddress2,SNMPTrapPort2);
        sprintf(TrapAddress3, "trapsink %s:%-16s\n", SNMPTrapAddress3,SNMPTrapPort3);
        sprintf(TrapAddress4, "trapsink %s:%-16s\n", SNMPTrapAddress4,SNMPTrapPort4);
        sprintf(TrapAddress5, "trapsink %s:%-16s\n\n", SNMPTrapAddress5,SNMPTrapPort5);
    }
    sprintf(OperationName, "sysoperationname %s\n",sysOperationName);
    sprintf(NetElementType,"sysnetelementtype %s\n",sysNetElementType);
    sprintf(NetElementName,"sysnetelementname %s\n\n",sysNetElementName);

    /* 以下写入文件中,如果是创建文件，从文件头处写；*/
                      
    if (writeconfileflag == 0)
    {
        fwrite(confText,strlen(confText),1,fp1);

        fwrite(warnText,strlen(warnText),1,fp1);
        fwrite(TrapCommunity,strlen(TrapCommunity),1,fp1);
        fwrite(TrapAddress1,strlen(TrapAddress1),1,fp1);
        fwrite(TrapAddress2,strlen(TrapAddress2),1,fp1);
        fwrite(TrapAddress3,strlen(TrapAddress3),1,fp1);
        fwrite(TrapAddress4,strlen(TrapAddress4),1,fp1);
        fwrite(TrapAddress5,strlen(TrapAddress5),1,fp1);
        fwrite(OperationName,strlen(OperationName),1,fp1);
        fwrite(NetElementType,strlen(NetElementType),1,fp1);
        fwrite(NetElementName,strlen(NetElementName),1,fp1);
        fclose(fp1);

        return 1;
    }

    /*  如果是更新文件，先读取原有内容，然后以只写模式打开重写文件 */
    memset(tsnmpSyndata.confText,0,MAX_SNMPCONFTEXT_LEN);
    while (fgets(line, sizeof(line), fp1) != NULL) 
    {
        cptr = line;

        /* find the token == "trapcommunity" or not   */
        if ((cptr = skip_white(cptr))) 
        {
            cptr = copy_nword(cptr, token, sizeof(token));
                
            if (!strcmp(token,"trapcommunity"))
            {
                break;
            }
            else
            {
                if(strlen(tsnmpSyndata.confText) + strlen(line) < MAX_SNMPCONFTEXT_LEN)
                    strcat(tsnmpSyndata.confText,line);
            }
        }
        else
        {
    ...
    fwrite(tsnmpSyndata.confText,strlen(tsnmpSyndata.confText),1,fp1);
    fwrite(TrapCommunity,strlen(TrapCommunity),1,fp1);
    fwrite(TrapAddress1,strlen(TrapAddress1),1,fp1);
    fwrite(TrapAddress2,strlen(TrapAddress2),1,fp1);
    fwrite(TrapAddress3,strlen(TrapAddress3),1,fp1);
...
CreateConfigFile()
{
    ...
    snprintf(configfile, sizeof(configfile),
        "%s/%s.conf", confpath, "snmpd");

    mkdirhier(configfile, NETSNMP_AGENT_DIRECTORY_MODE, 1);
    if((fp1 = fopen(configfile,"r")) != NULL)
    {
        fclose(fp1);

       /* 检查conf文件中trap token项 */
	CheckConfile_trapsinks(configfile);
        return;
    }
	
    if(WriteConfigFile(configfile) < 0)
    {
        OAM_Printf(PRINT_OAM_SNMP,PRN_NORMAL,PRN_LEVEL_HIGHEST,
                   "SNMPAgt():can not create the default config file!\n");
    }
}
...
RecvSynConfigFile()
{
    ...
    snprintf(configfile, sizeof(configfile),
        ...
        if((fp1 = fopen(configfile,"w")) == NULL)
        {   
           OAM_Printf(PRINT_OAM_SNMP,PRN_NORMAL,PRN_LEVEL_HIGHEST,
				"SNMPAgt():update slave board snmpd.conf fail!\n");
            return;
        }

       fwrite(snmpconfText,strlen(snmpconfText),1,fp1);
	fclose(fp1);
...
* 其它说明：CRDCR00125016 为备板使用
* 修改日期      版本号  修改人      修改内容
* ---------------------------------------------------------------------
* 2008/01/22    V1.0    安志奇      创建
************************************************************************/
void
reReadConfigFile(char *confText)
{
    FILE *fp1 = NULL;
    const char     *confpath;
    char            configfile[300];
    ...
    snprintf(configfile, sizeof(configfile),
        ...
        {   
           OAM_Printf(PRINT_OAM_SNMP,PRN_NORMAL,PRN_LEVEL_HIGHEST,
				"SNMPAgt():read snmpd.conf fail!\n");
            return;
        }

        while (fgets(line, sizeof(line), fp1) != NULL) {
            if(strlen(confText) + strlen(line) < MAX_SNMPCONFTEXT_LEN)
                strcat(confText,line);
        }
        fclose(fp1);
}

/**********************************************************************
* CheckConfile_trapparameters
* 功能描述：检查配置文件snmpd.conf中是否有trap2sink或trapsink配置参数，
            。如果没有或不足5个，补齐。
            这个默认是支持5个地址的在线配置，修改，主备同步。
* 访问的表：无
* 修改的表：无
* 输入参数：
            无
...
* 修改日期      版本号  修改人      修改内容
* ---------------------------------------------------------------------
* 2008/01/5    V1.0    安志奇      创建
************************************************************************/
void CheckConfile_trapsinks(const char *filename)
{
    FILE *fp1;
    int trapsink_count = 0,i;
    int trapcommunity_count = 0;
    char *TrapAddress = "trap2sink 10.0.0.0:162            \n";
    char *Trapcommunity = "trapcommunity public\n";
    ...
    while (fgets(line, sizeof(line), fp1) != NULL) {
        ...
        /* find the token == "trapsink" or "trap2sink"   */
        if ((cptr = skip_white(cptr))) 
        {
            cptr = copy_nword(cptr, token, sizeof(token));
            ...
            if (!strcmp(token,"trap2sink") || !strcmp(token,"trapsink"))
            {
                trapsink_count++;
            }
        }
    ...
    if (!trapcommunity_count) /* snmpd.conf中没有trapcommunity token */
    {
        fwrite(Trapcommunity,strlen(Trapcommunity),1,fp1);
    }
    for (i=0;i<(5-trapsink_count);i++)
    {
        fwrite(TrapAddress,strlen(TrapAddress),1,fp1);
    }
...
void ReadConfigFile(char *filename)
{
	...
	while (fgets(line, sizeof(line), fp1) != NULL) 
	{
        ...
        if ((cptr = skip_white(cptr))) 
        {
            ...
            if (!strcmp(token,"sysoperationname"))
            {
                if ((cptr = skip_white(cptr))) 
                {
                   	cptr = copy_nword(cptr,sysOperationName,sizeof(sysOperationName));
                }
            }
			if (!strcmp(token,"sysnetelementtype"))
            {
                if ((cptr = skip_white(cptr))) 
                {
                   	cptr = copy_nword(cptr,sysNetElementType,sizeof(sysNetElementType));
                }
            }
			if (!strcmp(token,"sysnetelementname"))
            {
                if ((cptr = skip_white(cptr))) 
                {
                   	cptr = copy_nword(cptr,sysNetElementName,sizeof(sysNetElementName));
                }
            }
