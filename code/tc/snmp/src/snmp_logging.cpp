 ...
 * * Contributed by Ragnar Kj鴕stad, ucd@ragnark.vestdata.no 1999-06-26 
 */
...
/*add by caoliang
static BYTE      g_SnmpPrnLevel = LOG_ERR;  */
...
snmp_enable_filelog(const char *logfilename, int dont_zero_log)
{
    ...
    if (logfilename) {
        mkdirhier(logfilename, NETSNMP_AGENT_DIRECTORY_MODE, 1);
...
/**********************************************************************
* OAM_SnmpLogLevel()
* 功能描述：SNMP Agent日志写入等级调整函数
* 访问的表：无
* 修改的表：无
* 输入参数：BYTE ucLevel---日志等级，范围从LOG_EMERG --- LOG_DEBUG
* 输出参数：无
* 返 回 值：无
* 其它说明：无
* 修改日期      版本号  修改人      修改内容
* ---------------------------------------------------------------------
* 2009/02/16    V1.0    曹亮      创建
************************************************************************/
void OAM_SnmpLogLevel(BYTE ucLevel)
{
    if(ucLevel > LOG_DEBUG)
        printf("it's wrong param!\n");
    g_SnmpPrnLevel = ucLevel;
}
#endif
/**  @} */
