...
#if 0
    opttind = 1;
    while ((arg = getoptt(argc, argv, Opts)) != EOF) {
...
#define SNMPV3_CMD_OPTIONS
        ...
        case 'Z':
             ...
             */
            else if (opttind < argc) {
                session->engineTime = strtoul(argv[opttind], &cp, 10);
                if (errno || cp == argv[opttind]) {
                    fprintf(stderr, "Need engine time after \"-Z engineBoot\".\n");
                    return (-1);
                }
            } else {
    ...
    session->peername = argv[opttind++]; /* hostname */
