...
get_paraForName(char *name)
{
    struct targetParamTable_struct *ptr;
    for (ptr = aPTable; ptr != NULL; ptr = ptr->next) {
        if (ptr->paramName && strcmp(ptr->paramName, name) == 0)
            return ptr;
    }
    return NULL;
}
...
void
snmpTargetParamTable_remove(struct targetParamTable_struct *oldEntry)
{
    snmpTargetParamTable_remFromList(oldEntry,&aPTable);
}
