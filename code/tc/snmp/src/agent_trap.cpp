...
/*  Send Trap Flag.if is 0,not send trap */
int snmp_trap_enable = 1;
extern int snmpNotifyVersion;
		     ...
		     char *com, int version, int pdutype)
{
    ...
    if ((NULL == netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID,
        ...
        strcpy(session.localname,"localhost");
...
int get_trapEnable()
{
    return snmp_trap_enable;
}

void set_trapEnable(int Enablevalue)
{
    snmp_trap_enable = Enablevalue;
}
...
snmpd_parse_config_trapsink(const char *token, char *cptr)
{
    ...
    snmpNotifyVersion = SNMP_VERSION_1;
}
#endif
...
snmpd_parse_config_trap2sink(const char *word, char *cptr)
{
    ...
    snmpNotifyVersion = SNMP_VERSION_2c;
}
...
snmpd_parse_config_trapsess(const char *word, char *cptr)
{
    ...
    if (ss->version == SNMP_VERSION_3 &&
        ...
        ss->securityEngineIDLen == 0) {
            len = snmpv3_get_engineID( (u_char*)tmp, sizeof(tmp));
            memdup(&ss->securityEngineID, (u_char*)tmp, len);
 ...
 */
char *
get_trapcommunity(void)
{
    if(snmp_trapcommunity != NULL)
	return snmp_trapcommunity;
    else
        return NULL;
}
