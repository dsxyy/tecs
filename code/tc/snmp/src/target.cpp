                    ...
                    void *filterArg)
{
    ...
    char            buf[MAX_BUFFER];
    char            tags[MAX_TAGS][MAX_BUFFER], *cp;
    int             numtags = 0, i;
            ...
            for (cp = targaddrs->tagList; cp;) {
                ...
                for (i = 0; i < numtags; i++) {
                    if (strcmp(buf, tags[i]) == 0) {
                        ...
                        if (targaddrs->sess) {
							/*Modify by Caoliang
						    char *pVersion = Get_cmmTrapVersion();
							int nVersion = 0;
							if(!strcmp(pVersion,"v2"))
								nVersion = 1;
							targaddrs->sess->version = nVersion;
							
							targaddrs->sess->community = Get_cmmTrapCommunity();
							targaddrs->sess->community_len = strlen(Get_cmmTrapCommunity());*/
