...
class port:
    ...
    def set_netplane(self,cluster_name,host_name,port_name,netplane):
        result = self.apisvr.tecs.port_netplane.set(self.session,cluster_name,host_name,port_name,netplane)
        if result[0] != 0:
            print "fail to set %s port_netplane of %s host in %s cluster! error code = %d" % (port_name,host_name,cluster_name,result[0])     
            print "parameter:"
            for n,v in port_paras.items():                
                print n,v
            print "error string = %s" % result[1]
            ...
            print "succeed to set %s port_netplane of %s host in %s cluster!" % (port_name,host_name,cluster_name)
            return True
