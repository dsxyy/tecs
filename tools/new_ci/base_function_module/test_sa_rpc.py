...
# 测试接口封装
#from interface.base_interface.rpc import sa

class UnitTestSaDisable(unittest.TestCase):
    '''
    设置磁阵进入维护模式(tecs.sa.disable)
    '''  
...
class UnitTestSaEnable(unittest.TestCase):
    '''
    设置磁阵退出维护模式(tecs.sa.enable)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
class UnitTestSaForgetById(unittest.TestCase):
    '''
    通过ID将已注销磁阵从SC中遗忘掉(tecs.host.forgetbyid)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
class UnitTestSaForgetByName(unittest.TestCase):
    '''
    通过名字将已注销磁阵从SC中遗忘掉(tecs.host.forgetbyname)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
class UnitTestSamapcluster(unittest.TestCase):
    '''
    插入集群和磁阵的关系(tecs.sa.mapcluster)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
def all_test_entry(environment):
    UnitTestSaDisable(environment).test_entery()
    UnitTestSaEnable(environment).test_entery()
    UnitTestSaForgetById(environment).test_entery()
    UnitTestSaForgetByName(environment).test_entery()
    UnitTestSamapcluster(environment).test_entery()


# 模块文档输出总入口
def all_doc_entry(cidoc):
    # 保存在EXCEL中的sheet标签名称定义
    module_name = "存储sa管理RPC"
    cidoc.write_doc(UnitTestSaDisable, module_name)
    cidoc.write_doc(UnitTestSaEnable, module_name)
    cidoc.write_doc(UnitTestSaForgetById, module_name)
    cidoc.write_doc(UnitTestSaForgetByName, module_name)
    cidoc.write_doc(UnitTestSamapcluster, module_name)
