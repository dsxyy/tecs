...
# 测试接口封装
#from interface.base_interface.rpc import snmp

class UnitTestSnmpConfigQuery(unittest.TestCase):
    '''
    查询SNMP基本配置(tecs.snmpconfiguration.query)
    '''  
...
class UnitTestSnmpConfigSet(unittest.TestCase):
    '''
    配置SNMP基本配置(tecs.snmpconfiguration.set)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
class UnitTestSnmpUserQuery(unittest.TestCase):
    '''
    查询SNMP XML-RPC接口用户名和密码(tecs.snmpuser.query)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
class UnitTestSnmpUserSet(unittest.TestCase):
    '''
    设置SNMP XML-RPC接口用户名和密码(tecs.snmpuser.set)
    '''  
    def __init__(self, environment):
        self.environment = environment
        self.config_para = self.environment.config_para
...
def all_test_entry(environment):
    UnitTestSnmpConfigQuery(environment).test_entery()
    UnitTestSnmpConfigSet(environment).test_entery()
    UnitTestSnmpUserQuery(environment).test_entery()
    UnitTestSnmpUserSet(environment).test_entery()


# 模块文档输出总入口
def all_doc_entry(cidoc):
    # 保存在EXCEL中的sheet标签名称定义
    module_name = "snmp接口 RPC"
    cidoc.write_doc(UnitTestSnmpConfigQuery, module_name)
    cidoc.write_doc(UnitTestSnmpConfigSet, module_name)
    cidoc.write_doc(UnitTestSnmpUserQuery, module_name)
    cidoc.write_doc(UnitTestSnmpUserSet, module_name)
