...
CHAR* MDString(CHAR *string)
{
    ...
    static  CHAR output[MAX_OAM_USER_PWD_ENCRYPT_LEN]={0};

    memset(output, 0, sizeof(output));
	
    SHA1Init(&ctx);
    SHA1Update(&ctx, (const uint8*)string, strlen(string));
    SHA1Final(md_value, &ctx);
   
    for(unsigned int i = 0; i<SHA1_RESULTLEN; i++)
    {
        XOS_snprintf(output, sizeof(output), "%s%02x", output, (unsigned short) md_value[i]);
    }
...
#endif
#if 0
string User::Sha1Digest(const string& pass)
{
    ...
    SHA1Init(&ctx);
    SHA1Update(&ctx, (const uint8 *)pass.c_str(), pass.length());
    SHA1Final(md_value, &ctx);
