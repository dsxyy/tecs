<?php
    ...
    $filed = array("image_id","file_name","register_time",$enabled_type,$is_public_type,"running_vms",$container_format_type,"size", "os_type", "description");
    $orderfield = "";
    $orderdir = "";
    $order = "";
    ...
    $totalnumrs->Close();
    //查表数据
    $query = ' SELECT image_id,file_name,register_time,' . $enabled_type . ',' . $is_public_type . ',running_vms,' . $container_format_type .',size, os_type,description,container_format, ';
    ...
    while (!$rs->EOF) {
        ...
        if($rs->fields[10] == "tpi"){
            ...
            $arrData[] = $rs->fields[8];//os_type
        }
        $arrData[] = $rs->fields[9];//description
        $arrData[] = $rs->fields[10];//container_format
        $arrData["workflow"] = $rs->fields[11];
