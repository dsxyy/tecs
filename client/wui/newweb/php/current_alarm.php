<?php
    ...
    $filed = array("alarm_id","alarm_code","alarm_time","lgclocation");
    $orderfield = "";
    $orderdir = "";
    $order = "";
    ...
    if($sSearch != ""){
        $filter = $filter . " and ( ";
        $filter = $filter . " to_char(alarm_id,'99999999999999999') like " . "'%" . $sSearch . "%'";
        $filter = $filter . " or to_char(alarm_code,'99999999999999999') like " . "'%" . $sSearch . "%'";
    ...
    //表记录数
    $query = "SELECT COUNT(*) FROM web_view_current_alarm WHERE " . $filter;
    ...
    $totalnumrs->Close();
    //查表数据
    $query = ' SELECT alarm_id,alarm_code,alarm_time,lgclocation,addinfo';
    $query = $query . ' FROM web_view_current_alarm';
    ...
    while (!$rs->EOF) {
        $arrData = Array();
        $arrData[] = $rs->fields[0];//alarm_id
        $arrData[] = $rs->fields[1];//alarm_code
        $arrData[] = $rs->fields[2];//alarm_time
        $arrData[] = $rs->fields[3];//lgclocation
        $addinfo   = $rs->fields[4];
        $arrData[] = str_replace("\n","",$addinfo);//告警详情
        $arrData[] = "";//告警原因
        $arrData[] = "";//建议
